...
def visualizar_devolucoes_filtradas():
    conn = conectar_bd()
    cursor = conn.cursor()

    print("\nDeseja aplicar algum filtro?")
    print("1. Ver todas as devoluções")
    print("2. Filtrar por nome do usuário")
    print("3. Filtrar por título do livro")
    print("4. Filtrar por intervalo de datas de devolução")
    print("0. Cancelar")
    ...
    if escolha == "2":
        ...
        filtros.append("DATE(e.data_devolucao) BETWEEN ? AND ?")
        parametros.extend([data_inicio, data_fim])

    if filtros:
        query_base += " AND " + " AND ".join(filtros)

    query_base += " ORDER BY e.data_devolucao DESC"
    ...
    if not resultados:
        print("Nenhuma devolução encontrada com os filtros aplicados.")
        return

    print("\nDevoluções encontradas:")
    print("-" * 70)
    for emp_id, nome_usuario, titulo, autor, data_emp, data_dev in resultados:
        print(f"ID: {emp_id} | Usuário: {nome_usuario} | Livro: {titulo} - {autor}")
        print(f"  → Emprestado em: {data_emp} | Devolvido em: {data_dev}")
    print("-" * 70)
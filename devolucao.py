...
def obter_emprestimos_ativos(usuario_id):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT e.id, l.id, l.titulo, l.autor, e.data_emprestimo
        FROM emprestimos e
...
def registrar_devolucao(usuario_id_logado):
    papel = obter_papel_usuario(usuario_id_logado)

    if papel not in ["bibliotecario", "administrador"]:
        print("Você não tem permissão para registrar devoluções.")
        return
    ...
    print("\nEmpréstimos ativos:")
    for idx, (emprestimo_id, livro_id, titulo, autor, data_emprestimo) in enumerate(emprestimos, 1):
    ...
    emprestimo_id, livro_id, _, _, _ = emprestimos[escolha - 1]
